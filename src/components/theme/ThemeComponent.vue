<template>
  <div>
    <space class="background-space ">
      <div class="dropdown "><button class="dropbtn ">Background Color<span class="material-icons large "> brush </span>
        </button>
        <select class="color-dropbtn " v-model="selectedColor">
          <option class="red ">Maroon</option>
          <option class="green ">Green</option>
          <option class="grey ">Grey</option> 
          ...
          <option class="darkslategrey "> Darkslategrey</option>
        </select>
  ...
  methods: {
    postColor() {
      const res = axios.put(BASE_URL + "/api/backgroundsettings", {
        id: 1, //we did id 1 because we are updating data but the is no selected data so we have to give id like this because in the other case we are always creating new data(with new id) with put method
    ...
    uploadLogo(e) {
      ...
      this.noLogo = !e.target.files.length //noLogo is empyt target.file because our add button will be disabled when there is no file in file input(we did like this for don't send null/empty data)
